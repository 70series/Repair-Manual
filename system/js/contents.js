 ...
 *  2017/01/05|1.2.0|森|REL_00313<br />
...
// 2017/08/31 add start ver1.3.0
    ...
    // 手順SeePage見せ方改善
    // ---------------------
    // クエリ文字列に「絞込み対象の参照先ID」が指定されている場合
    if (objQuary.filter_refid) {
      // 手順表示の絞り込み処理を呼び出す
      Contents.$filterProcedures(objQuary.filter_refid);
    }
// 2017/08/31 add end ver1.3.0
    ...
    Util.$propcopy(
        ...
        Contents.mySuccession);
    if(Contents.myParent.globalInfo) {
      Contents.globalInfo.PUB_BIND_ID = Contents.myParent.globalInfo.PUB_BIND_ID;
      Contents.globalInfo.BRAND       = Contents.myParent.globalInfo.BRAND;
      Contents.globalInfo.BRAND_NAME  = Contents.myParent.globalInfo.BRAND_NAME;
      Contents.globalInfo.FROM_DATE   = Contents.myParent.globalInfo.FROM_DATE;
      Contents.globalInfo.EWD_TEKI_DATE = Contents.myParent.globalInfo.EWD_TEKI_DATE;
    }
    //項目の追加
...
// ---------------------
/**
 * 手順表示の絞り込み処理 (指定された手順のみを表示し、その他の手順は非表示にする)
 * @param {string} filterRefId 絞込み対象の参照先ID
 */
Contents.$filterProcedures = function(filterRefId) {

  var procedureList;
  var procedureCount;
  var procedureElement;

  var resultDescendantList;

  // 指定された参照先IDが、手順IDであることを確認する。
  // 参照先IDの長さが23文字であれば、手順IDである。
  // もし指定された参照先IDが手順IDではない場合は、何もせずに処理を抜ける。
  if (filterRefId.length != 23) return;

  // 指定された参照先IDに対応する手順が、パラグラフ内に存在していることを確認する。
  // もし該当手順が存在しない場合は、何もせずに処理を抜ける。
  if ($(filterRefId) === null) {
    return;
  }

  // 手順の一覧を取得する。
  procedureList = $$("div.s1, div.testgrp");
  procedureCount = procedureList.length;

  // 取得した手順を、一つずつ処理する。
  for (var i = 0; i < procedureCount; ++i) {
    procedureElement = procedureList[i];

    // a) 手順IDが、指定された手順IDと一致する場合 (= 唯一表示させる手順))
    if (procedureElement.id === filterRefId) {

      // 手順はそのまま表示するが、
      // 手順配下の診断結果部分は非表示にする。

      // 手順配下の診断結果部分を取得する。
      resultDescendantList = Util.Selector.$select(
        "div.judgeValueNext, div.nextAction, div.judgeValueEnd, div.endActBox",
        procedureElement);

      // 取得した診断結果部分を非表示にする。
      Contents.$hideElements(resultDescendantList);

    // b) 手順IDが、指定された手順IDと一致しない場合 (= 非表示にする手順)
    } else {
      // 手順を非表示にする。
      Element.$hide(procedureElement);
    }
  }
};

/**
 * 指定されたHTML要素(複数可)を、非表示にする。
 * このメソッドで非表示にしたHTML要素は「手順展開処理」の影響を受けない。
 * @param {elements} elementList 非表示対象の手順要素群
 */
Contents.$hideElements = function(elementList) {
  var elementCount;
...
Contents.$processForRMAndNCF = function() {
  ...
  try {
    ...
    var linkKey  = "";
    var ewdlinks   = $$("a.ewd_mlink");
    var ewdlinkLen = ewdlinks.length;
    var spanEls  = null;
    var spanLen  = 0;
    
    // ewd_mlink数分だけループする
    for(var i = 0; i < ewdlinkLen; i++) {
      spanEls = Util.Selector.$select("span", ewdlinks[i]);
      spanLen = spanEls.length;
      linkKey = Util.$getNodeText(spanEls[0]);
      
      // 表示モードが"1"以外でリンクキーがある場合はイベント設定
      if(linkKey && (Contents.showMode != "1")) {
        Use.Util.$observe(ewdlinks[i], "click",
          (function(key) {
            return function() { Contents.$doClickEWDLnk(key); };
          })(linkKey)
        );
      // リンクキーが無い場合はewd_mlinkを非表示にする
      } else if(!linkKey) {
        Element.$addClassName(ewdlinks[i], "invisible");
      }
    }
...
Contents.$processForOtherManual = function() {
  ...
  try {
    ...
    for(var i = 0; i < len; i++) {
      ...
      if(span.innerHTML.substring(0,1)=="/"){
        pathFor=path;
      }else{
        pathFor="";
      }
      ...
      Use.Util.$observe(els[i], "click",
        ...
        })(pathFor + span.innerHTML, target, option, windowSize)
      );
    }
...
// 2017/08/31 add start ver1.3.0
      ...
      // 手順SeePage見せ方改善
      // ---------------------
      // アンカー (URLの「#」より後ろの部分) が存在する場合、
      if (anc.substring(1) !== "") {
        // アンカーより後ろの部分を「絞込み対象の参照先ID」として URLに付与する。
        path += "&filter_refid=" + anc.substring(1);
      }
// 2017/08/31 add end ver1.3.0
      path += anc;
 ...
 * @param {string} linkKey リンクキー
 */
Contents.$doClickEWDLnk = function(linkKey) {
  var METHODNAME = "Contents.$doClickEWDLnk";
  try {
    var cG = Contents.globalInfo;
    ...
    // 修理書と配線図の適用時期ズレを考慮し、
    // 配線図タブの持つ過去直近のEWD適用時期取得
    var ewdTermDate = Contents.$getEWDTermDateFromTabInfo();

    // 配線図リンクで設定されたLINKKEYに対して、
    // 車種世代、EWD適用時期、表示言語の情報を追加
    cG.LINKKEY     = "pubBind=" + Contents.globalInfo.PUB_BIND_ID           + ";" + 
                     "term="    + ewdTermDate                               + ";" + 
                     "vwlang="  + Util.$getAttrValue($$("html")[0], "lang") + ";" + 
                     linkKey;

    var tab = Contents.tabName;
    var newConfig = ["10", "11"];
    
    Util.$openUrl("../../../pgm_ewdviewer/service.html", "_blank", Contents.WINDOW_OPTION, Util.WINDOW_SIZE_1);
...
Contents.$getEWDTermDateFromTabInfo = function() {

  var ewdTermDate = "";

  try {

    ewdTermDate = Contents.globalInfo.EWD_TEKI_DATE;

    if (ewdTermDate === "") {
      ewdTermDate = Contents.globalInfo.FROM_DATE;
    }

    return (ewdTermDate === null) ? "" : ewdTermDate;
