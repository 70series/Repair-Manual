 ...
 * @param {object} doc DOM ROOTオブジェクト(pub.xml)
 */
Top.$setPubFormat = function(doc) {
  var METHODNAME = "Top.$setPubFormat";
  try {
    
    var ele = Util.$getSingleNode(doc, Top.XPATH_LIST.PUB_FORMAT);
    var txt = null;
    ...
    if(ele != null) {
      ...
      //format要素の値を取得
      txt = Util.$getNodeText(ele);

      //format要素の値が"SIAS2.1"の場合
      if(txt == "SIAS2.1") {
        Top.globalInfo.PUB_FORMAT = "1";

      //上記以外の場合
      } else {
        Top.globalInfo.PUB_FORMAT = "0";
      }
...
Top.$setPublishingInfo = function(strTekiDate) {
  ...
  try {
    ...
    // SIAS2.1の場合、横串検索ボタンを非活性にする
    if(Top.globalInfo.PUB_FORMAT == "1") {
      // 横串検索ボタンを非活性にする
      strValidYoko = "no";
    }
