...
// 2017/08/31 add start ver1.3.0
        ...
        if (nowHistory.FILTER_REFID) {
          url += "&filter_refid=" + nowHistory.FILTER_REFID;
        }
// 2017/08/31 add end ver1.3.0
        tgt += "TechnoMode";
...
// 2017/08/31 add end ver1.3.0
            ...
            // 表示用のURLは通常のコンテンツ
            url += refId ? "#" + refId : "";
          // カテゴリが"C"、"Y"、"J"のいずかの場合、診断パラグラフと判断
          // パブ形式の値が"0"(SIAS2)の場合、診断フローと判断
          } else if(this.myGlobalInfo.PUB_FORMAT == "0") {
            // スクロールバーを出さない
            scr = "no";
...
// 2017/08/31 add end ver1.3.0
          ...
          // パブ形式の値が"0"(SIAS2)以外の場合、診断フローと手順一覧が混在
          // 手順一覧の場合
          } else if(Service.Repair.$isList(
              Service.Repair.DIAGFLW_PATH.replace("{0}", parId)) == "1") {
            // スクロールバーを自動に設定
            scr = "auto";
            // 履歴用のURLは診断フローのパラグラフ（意見要望で表示する内容）
            url = Service.Repair.CONTENT_PATH.replace("{0}", parId);
...
// 2017/08/31 add start ver1.3.0
              // 絞込み対象の参照先ID (手順SeePage見せ方改善)
              , own.getSelectInfo().FILTER_REFID
// 2017/08/31 add end ver1.3.0
            );
          }
        })(this)
...
// 2017/08/31 add start ver1.3.0
    ...
    // 手順SeePage見せ方改善
    // ---------------------
    // 要領参照リンク(手順間参照リンク)経由でパラグラフを表示する場合は、
    // 手順表示の絞り込みを行う。
    newHistory.FILTER_REFID = refKey;
// 2017/08/31 add end ver1.3.0
 ...
 * 診断パラグラフの判定処理
 * @private
 * @param {string} url コンテンツHTMLのパス
 * @return {string} result 判定結果 ("1"：手順一覧、"0"：それ以外)
 */
Service.Repair.$isList = function(url) {
  var METHODNAME = "Service.Repair.$isList";
  try {
    ...
    Use.Util.$request(
      url,
      false,
      function(res) { Service.Repair.$getHtmlOnSuccess(res, doc); },
      Service.Repair.$getHtmlOnFailure,
      true,
      false
    );
    
    if (Util.Selector.$select("div#list_frame", doc).length > 0) {
      result = "1";
    } else {
      result = "0";
    }
 ...
 * @param {Object}   doc    HTMLのDOM
 */
Service.Repair.$getHtmlOnSuccess = function(res, doc) {
  var METHODNAME = "Service.Repair.$getHtmlOnSuccess";
  try {
 ...
 * @param {Response} res レスポンスオブジェクト
 */
Service.Repair.$getHtmlOnFailure = function(res) {
  var METHODNAME = "Service.Repair.$getHtmlOnFailure";
  try {
