 ...
 *  2016/11/17|1.2.0|森|REL_00313<br />
 ...
 * @param {string} gtsFunction GTS機能ID
 * @param {string} gtsEcu ECUID
 * @param {string} gtsUtil ユーティリティID
 * @param {function} funonErrorConnectServer ConnectServer失敗時のコールバック処理
 */
 Use.GTS.$showGtsPlus = function(faceBox, carInfo, gtsFunction, gtsEcu, gtsUtil, funonErrorConnectServer) {

  // sha256_a.js読込完了時処理
  var funonOnloadSha256A = function(e) {
    // GTS+連携用クラスオブジェクト生成
    var objGtsPlus = new SIAS2.Gts.Common();

    // GTS+連携エラー発生時のコールバック関数
    var funonError = function(apiId, resultInfo, apiParam, errCode, message) {
      // サーバー接続を解除し、オブジェクトを破棄する
      objGtsPlus.disconnectServer(function(){}, function(){});
      objGtsPlus = null;

      if (errCode == "0x8004024A") {
        faceBox.close();
      } else {
        // apiIdとAPI実行エラーを"_"で連結して渡す
        var gtsplusApiResult = apiId + "_" + resultInfo.code;
        Use.GTS.$showMessage(errCode, gtsplusApiResult, gtsFunction);
        faceBox.close();
      }
    };

    // GUIオープン処理コールバック関数
    var funonExecuteGuiRelay = function() {
      // サーバー接続を解除し、オブジェクトを破棄する
      objGtsPlus.disconnectServer(function(){}, function(){});
      objGtsPlus = null;
    ...
    // 接続初期化処理コールバック関数
    var funonInitializeConnect = function() {
      //GTS連携Techstream起動OK,機能起動中状態表示
      Use.GTS.$showStatus("MVWF0022AAI", "MVWF0024AAI", "MVWF0025AAI");

      // GUIオープン処理呼出し
      objGtsPlus.executeGuiRelay(
        objGtsPlus.createGuiRelayArgData(gtsFunction, gtsEcu, gtsUtil),
        funonExecuteGuiRelay
      );
    };

    // サーバー接続処理コールバック関数
    var funonConnectServer = function() {
      //GTS連携準備OK,Techstream起動中状態表示
      Use.GTS.$showStatus("MVWF0022AAI", "MVWF0023AAI", "");

      // 仕向をGTS+の仕向IDに変換する
      var carInfo2 = carInfo;
      var regionIdWork = carInfo.FOR_LANG;
      carInfo2.FOR_LANG = Use.GTS.$serviceRegionIdToGTSRegionId(regionIdWork);
  
      // 接続初期化処理を呼出し
      objGtsPlus.initializeConnect(
        carInfo2,
        gtsEcu,
        funonInitializeConnect,
        funonError
      );
    };

    // サーバー接続処理呼出し
    objGtsPlus.connectServer(
      funonConnectServer,
      function(){},
      funonErrorConnectServer,
      function(){},
      function(){},
      function(){},
      function(){},
  ...
  // use_gtsplus_ws.js読込完了時処理
  var funonOnloadScriptUseGtsPlusWs = function(e) {
    // sha256_a.jsを動的読込
    var scriptSha256A = document.createElement("script");
    scriptSha256A.type = "text/javascript";
    scriptSha256A.src = "../../../system/js/sha256_a.js";
    scriptSha256A.onload = funonOnloadSha256A;

    // ページ内のheadタグ取得
    var headTag = document.getElementsByTagName("head");
    // headタグにスクリプトタグを追加
    headTag[0].appendChild(scriptSha256A);
  }

  // use_gtsplus_api.js読込完了時処理
  var funonOnloadScriptUseGtsPlusApi = function(e) {
    // use_gtsplus_ws.jsを動的読込
    var scriptUseGtsPlusWs = document.createElement("script");
    scriptUseGtsPlusWs.type = "text/javascript";
    scriptUseGtsPlusWs.src = "../../../system/js/use_gtsplus_ws.js";
    scriptUseGtsPlusWs.onload = funonOnloadScriptUseGtsPlusWs;

    // ページ内のheadタグ取得
    var headTag = document.getElementsByTagName("head");
    // headタグにスクリプトタグを追加
    headTag[0].appendChild(scriptUseGtsPlusWs);
  };

  // use_gtsplus.js読込完了時処理
  var funonOnloadScriptUseGtsPlus = function(e) {
    // use_gtsplus_api.jsを動的読込
    var scriptUseGtsPlusApi = document.createElement("script");
    scriptUseGtsPlusApi.type = "text/javascript";
    scriptUseGtsPlusApi.src = "../../../system/js/use_gtsplus_api.js";
    scriptUseGtsPlusApi.onload = funonOnloadScriptUseGtsPlusApi;
    
    // ページ内のheadタグ取得
    var headTag = document.getElementsByTagName("head");
    // headタグにスクリプトタグを追加
    headTag[0].appendChild(scriptUseGtsPlusApi);
  };

  // use_gtsplus.jsを動的読込
  var scriptUseGtsPlus = document.createElement("script");
  scriptUseGtsPlus.type = "text/javascript";
  scriptUseGtsPlus.src = "../../../system/js/use_gtsplus.js";
  scriptUseGtsPlus.onload = funonOnloadScriptUseGtsPlus;
  
  // ページ内のheadタグ取得
  var headTag = document.getElementsByTagName("head");
  // headタグにスクリプトタグを追加
  headTag[0].appendChild(scriptUseGtsPlus);
 ...
 * @param {Facebox} faceBox フェースボックスインスタンス
 * @param {object(連想配列)} carInfo 車種選択情報
 * @param {boolean} isRecursion 再帰呼出しフラグ
 */
Use.GTS.$show = function(elements, faceBox, carInfo, isRecursion) {
  ...
  try {
    ...
    for(var eIdx = 0; eIdx < elmLen; eIdx++) {
      ...
      //属性が"gtsDtc"の場合，値を取得する
      } else if (elements[eIdx].className == "gtsDtc") {
        gtsDtc = elements[eIdx].innerHTML;
      }
    }

    if(isRecursion === undefined) {
      // GTS+が未インストール/未起動の場合の処理
      var funonErrorConnectServer = function() {
        // GTS連携・GTS画面表示を呼出し
        Use.GTS.$show(elements, faceBox, carInfo, true);
      };

      Use.GTS.$showGtsPlus(faceBox, carInfo, gtsFunction, gtsEcu, gtsUtil, funonErrorConnectServer);
      Use.GTS.vehicleInfo.IsError = true;
      // 画面ロックはコールバックで解除するため解除せずに終了
    ...
    ret = Use.GTS.$displayScreenCall(gtsPlugin, gtsFunction,
        ...
        gts2ndEcu, gts2ndPid, gtsMultiMode, gtsDtc, carInfo);
    //正常終了で無い場合
    if(ret != " ") {
...
Use.GTS.$displayScreenCall = function(
  ...
  try {
    ...
    ret = Use.GTS.$runDisplayScreenCall(gtsPlugin, functionId,
        ecuId, pidId, didId, customizeId, utilityId, 
        ecu2ndId, pid2ndId, multiMode, dtc);
...
/**
 * GTS連携・DisplayScreenCallの実行(バージョン分岐)
 * @param {object} gtsPlugin GTSプラグインオブジェクト
 * @param {string} functionId 機能ID
 * @param {string} ecuId EUC_ID
 * @param {string} pidId PID_ID
 ...
 * @return {object} 処理結果
 */
Use.GTS.$runDisplayScreenCall = function(
    gtsPlugin, functionId, ecuId, pidId, didId, customizeId, utilityId,
    ecu2ndId, pid2ndId, multiMode, dtc) {
  var METHODNAME = "Use.GTS.$runDisplayScreenCall";
  try {
    var callver = 0;

    // ・Multiモード対応にて、第2ECU_ID、第2PID_ID、マルチモードが追加された関数をVer2相当とする
    // ・FFD対応にて、DTCが追加された関数をVer3相当とする
    if (dtc != "") {
        callver = 3; // DisplayScreenCallFFD
    } else if (multiMode != "") {
        callver = 2; // DisplayScreenCallMulti
    } else {
        callver = 1; // DisplayScreenCall
    }

    //DisplayScreenCallFFD(Ver3相当)の実行
    if(callver >= 3) {
        try {
          if (multiMode == "") {
              multiMode = "0";
          }
          ret = gtsPlugin.DisplayScreenCallFFD(
                  functionId,
                  ecuId,
                  pidId,
                  didId,
    ...
    // DisplayScreenCallMulti(Ver2相当)の実行
    if(callver >= 2) {
        try {
...
Use.GTS.$showMessage = function(hResult, fncName, fncID) {
  ...
  try {
    ...
    //関数利用不可
      ...
      msgID = "MVWF0015AAE";
    // GTS+連携 未インストール/未起動
    } else if(hResult == "0x80040260") {
      msgID = "MVWF0141AAE";
    // GTS+連携 ダイアログユーティリティ実行中
    } else if(hResult == "0x80040261") {
      msgID = "MVWF0142AAE";
    // GTS+連携 CUW+実行中
    } else if(hResult == "0x80040262") {
      msgID = "MVWF0143AAE";
    // GTS+連携 GTS+車両選択処理タイムアウト
    } else if(hResult == "0x80040263") {
      msgID = "MVWF0144AAE";
    // GTS+連携 車両通信異常
    } else if(hResult == "0x80040264") {
      msgID = "MVWF0148AAE";
    // GTS+連携 車両突合せ結果不一致
    } else if(hResult == "0x80040265") {
      msgID = "MVWF0145AAE";
    // GTS+連携 サポート対象外ECU
    } else if(hResult == "0x80040266") {
      msgID = "MVWF0146AAE";
    // GTS+連携 ECU未接続
    } else if(hResult == "0x80040267") {
      msgID = "MVWF0147AAE";
    // GTS+連携 GUIリレータイムアウト
    } else if(hResult == "0x80040268") {
      msgID = "MVWF0148AAE";
    // GTS+連携 データモニタ/アクティブテスト実施中
    } else if(hResult == "0x80040269") {
      msgID = "MVWF0149AAE";
    // GTS+連携 GTS+車種選択排他制御中
    } else if(hResult == "0x8004026A") {
      msgID = "MVWF0150AAE";
    // GTS+連携 GTS+連携API内部エラー
    } else if(hResult == "0x8004026E") {
      msgID = "MVWF0148AAE";
    //(想定外エラー)
    } else {
      msgID = "MVWF0015AAE";
...
// 2017/08/31 add start ver1.3.0
    ...
    // 手順SeePage見せ方改善
    // ---------------------
    // 手順表示の絞り込みを行う場合、絞込み対象の参照先ID を contents.js 側に通知する。(URL経由)
    if (filterRefId) {
      url += "&filter_refid=" + filterRefId;
    }
// 2017/08/31 add end ver1.3.0
    if(anchor != "") {
      url += "#" + anchor;
